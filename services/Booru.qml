...
Singleton {
    ...
    property string failMessage: "Isso não funcionou. Dica:\n- Confira suas tags e configuração NSFW\n- Se você não tem uma tag em mente, tente um número de página"
    ...
    property var providers: {
        ...
        "yandere": {
            ...
            "description": "Genérico | Boa qualidade, quantidade decente",
            "mapFunc": (response) => {
        ...
        "konachan": {
            ...
            "description": "Para papéis de parede | Boa qualidade",
            "mapFunc": (response) => {
                return response.map(item => {
                    return {
                        "id": item.id,
        ...
        "zerochan": {
            ...
            "description": "Qualidade excelente, sem NSFW",
            "mapFunc": (response) => {
        ...
        "danbooru": {
            ...
            "description": "O mais popular | Maior quantidade, mas qualidade pode variar drasticamente",
            "mapFunc": (response) => {
                return response.map(item => {
                    return {
                        "id": item.id,
        ...
        "gelbooru": {
            ...
            "description": "Hentai | Boa quantidade, um monte de NSFW, qualidade vária muito",
            "mapFunc": (response) => {
        ...
        "waifu.im": {
            ...
            "description": "Apenas Waifus | Excelente qualidade, quantidade limitada",
            "mapFunc": (response) => {
        ...
        "t.alcy.cc": {
            ...
            "url": "https://t.alcy.cc",
            "api": "https://t.alcy.cc/",
            "description": "Imagens grandes | Qualidade suprema, sem NSFW.",
            "fixedTags": [
                {
                    "name": "ycy",
                    "count": "General"
    ...
    function setProvider(provider) {
        ...
        if (providerList.indexOf(provider) !== -1) {
            ...
            root.addSystemMessage(`Provedor definido como: ${providers[provider].name}. ${provider == "zerochan" ? ". Notas para Zerochan:\n- Você deve entrar uma cor\n- Definir o seu nome de usuário do zerochan em `sidebar.booru.zerochan.username`.  [Você pode ser bandio por não fazer isso](https://www.zerochan.net/api#:~:text=The%20request%20may%20still%20be%20completed%20successfully%20without%20this%20custom%20header%2C%20but%20your%20project%20may%20be%20banned%20for%20being%20anonymous.)!" : ""}`)
        } else {
            root.addSystemMessage("Fornecedor de API Inválido. Suportados: \n- ") + providerList.join("\n- ")
        }
    }
