...
Singleton {
    ...
    function handleRecognition(jsonText) {
        try {
            ...
            Quickshell.execDetached(["notify-send", "Não foi possível reconhecer a música", "Talvez o que você está escutando é muito niche", "-a", "Shell"])
        }
    }
    ...
    Process {
        ...
        onExited: (exitCode, exitStatus) => {
            if (exitCode === 1) {
                Quickshell.execDetached(["notify-send", "Não foi possível reconhecer a música", "Certifique-se que songrec está instalado", "-a", "Shell"])
            }
        }
    }
