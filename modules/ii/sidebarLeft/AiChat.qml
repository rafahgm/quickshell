...
Item {
    ...
    property var allCommands: [
        {
            name: "attach",
            description: "Anexe um arquivo. Funciona apenas com o Gemni",
            execute: args => {
        ...
        {
            name: "tool",
            description: "Defina a ferramenta para o modelo",
            execute: args => {
                // console.log(args)
                if (args.length == 0 || args[0] == "get") {
                    Ai.addMessage(`Usage: ${root.commandPrefix}tool TOOL_NAME`, Ai.interfaceRole);
                } else {
                    const tool = args[0];
                    const switched = Ai.setTool(tool);
                    if (switched) {
                        Ai.addMessage(`Ferramenta definida como: ${tool}`, Ai.interfaceRole);
                    }
                }
            }
        },
        {
            name: "prompt",
            description: "Defina o system prompt para o modelo",
            execute: args => {
                if (args.length === 0 || args[0] === "get") {
                    Ai.printPrompt();
                    return;
        ...
        {
            name: "key",
            description: "Configure uma chave de API",
            execute: args => {
                if (args[0] == "get") {
                    Ai.printApiKey();
                } else {
        ...
        {
            name: "save",
            description: "Salvar chat",
            execute: args => {
                const joinedArgs = args.join(" ");
                if (joinedArgs.trim().length == 0) {
                    Ai.addMessage(`Uso: ${root.commandPrefix}save CHAT_NAME`, Ai.interfaceRole);
                    return;
                }
        ...
        {
            name: "load",
            description: "Carregar chat",
            execute: args => {
                const joinedArgs = args.join(" ");
                if (joinedArgs.trim().length == 0) {
                    Ai.addMessage(`Uso: ${root.commandPrefix}load CHAT_NAME`, Ai.interfaceRole);
                    return;
                }
        ...
        {
            name: "clear",
            description: "Limpar histórico de chats",
            execute: () => {
                Ai.clearMessages();
            }
        },
        {
            name: "temp",
            description: "Defina a temperatura (aleatoriedade) do modelo. Valores entre 0 e 2 para o Gemini, 0 e 1 para os outros modelos. Padrão é 0.5.",
            execute: args => {
                // console.log(args)
                if (args.length == 0 || args[0] == "get") {
                    Ai.printTemperature();
    ...
    function handleInput(inputText) {
        if (inputText.startsWith(root.commandPrefix)) {
            ...
            if (commandObj) {
                ...
                Ai.addMessage(`Comando desconhecido: ${command}`, Ai.interfaceRole);
            }
        } else {
    ...
    ColumnLayout {
        ...
        Item {
            ...
            Rectangle {
                ...
                RowLayout {
                    ...
                    StatusItem {
                        ...
                        statusText: ""
                        description: Ai.currentModelHasApiKey ? "Chave de API definida\nAltere com /key YOUR_API_KEY" : "Sem chave de API\nDefina-a com /key YOUR_API_KEY"
                    }
                    StatusSeparator {}
                    StatusItem {
                        icon: "device_thermostat"
                        statusText: Ai.temperature.toFixed(1)
                        description: "Temperatura\nAltere com /temp VALUE"
                    }
                    StatusSeparator {
                    ...
                    StatusItem {
                        ...
                        description: `Total de tokens\nInput: ${Ai.tokenCount.input}\nOutput: ${Ai.tokenCount.output}`
                    }
                }
            }
            ...
            PagePlaceholder {
                ...
                icon: "neurology"
                title: "LLMs"
                description: "Digite /key para iniciar com modelos online\nCtrl+O para expandir a barra lateral\nCtrl+P para fixar a barra lateral\nCtrl+D para destacar a barra lateral"
        ...
        Rectangle { // Input area
            ...
            RowLayout { // Input field and send button
                ...
                StyledTextArea { // The actual TextArea
                    ...
                    placeholderText: `Envie "${root.commandPrefix}" para obter os comandos`
                        ...
                        if (messageInputField.text.length === 0) {
                            ...
                            root.suggestionList = promptFileResults.map(file => {
                                return {
                                    ...
                                    description: `Carregar o prompt de ${file.target}`
                                };
                            });
                            ...
                            root.suggestionList = promptFileResults.map(file => {
                                ...
                                return {
                                    ...
                                    description: `Salvar chat para ${chatName}`
                                };
                            });
                            ...
                            root.suggestionList = promptFileResults.map(file => {
                                ...
                                return {
                                    ...
                                    displayName: `${chatName}`,
                                    description: `Carregar chat de ${file.target}`
                                };
                            });
            ...
            RowLayout { // Controls
                ...
                ApiInputBoxIndicator {
                    ...
                    tooltipText: `Modelo atual: ${Ai.getModel().name}\nDefina com ${root.commandPrefix}model MODEL`
                }
                ...
                ApiInputBoxIndicator {
                    ...
                    tooltipText: `Ferramenta atual: ${Ai.currentTool}\nDefina com ${root.commandPrefix}tool TOOL`
                }
